...
def _():
    ...
    return da, dexpr, np, plt, pytest, time, xr
...
def _(mo):
    mo.md(r"""
    ## dask-array: let expression rewriting fuse it for you

    Everything above hand-builds the fusion — a `map_blocks` kernel, a `(sum, count)`
    collapse, a tree reduce — because stock dask can't rewrite `(x*1.8+32).mean()` into a
    map-reduce on its own. [dask-array](https://github.com/mrocklin/dask-array) is a
    reimplementation of dask's array layer that represents the calculation as an
    *expression* and optimises it (simplify → lower → fuse) before building the task
    graph. Crucially for us, its `NanMean` lowers to a `PartialReduce`, and the fusion
    pass folds the elementwise chain into that reduction's per-block leaves — the exact
    move `chunk-local` makes by hand, done structurally by the optimiser.

    So the strategy here is: **write the naive expression, change nothing else, and let
    the rewriter do the fusion.** No kernel, no `stats_chunks`, no `new_axis`. This is the
    closest off-the-shelf analogue to what an xrexpr `MapReduce` lowering would emit.

    `dask_array.xarray.register()` makes dask-array xarray's active chunk manager. It is
    opt-in and only affects arrays created *through* xarray's chunk machinery — every
    stock-dask case above holds `dask.array` objects built explicitly, so they keep using
    stock dask untouched. We back this case with the **same values** as `da_x` (routed
    through `dexpr.from_array`) so its mean agrees with the other rows to 1e-6.
    """)
    return


@app.cell
def dask_array_expr(da_x, dexpr, xr):
    # Switch xarray's "dask" chunk manager over to dask-array. Opt-in and scoped: it only
    # changes how *new* xarray chunks are made, so the stock-dask cases above (which hold
    # dask.array objects directly) are unaffected — a stock-backed .mean() stays stock.
    dexpr.xarray.register()

    # Same data as da_x, routed through dask-array so the value agrees with the other
    # rows to 1e-6. from_array on the persisted input's materialised values makes a
    # dask-array collection with the identical chunking.
    _chunks = tuple(axis[0] for axis in da_x.data.chunks)
    dexpr_x = xr.DataArray(
        dexpr.from_array(da_x.data.compute(), chunks=_chunks),
        dims=da_x.dims,
        name=da_x.name,
    ).persist()
    dexpr_x.compute()

    # The *naive* expression — no hand-built fusion. dask-array's optimiser lowers
    # NanMean -> PartialReduce and fuses the elementwise chain into it, so the graph that
    # actually runs is the map-reduce the other cells build by hand.
    dask_array_mean = ((dexpr_x * 1.8) + 32).mean()
    dask_array_mean.compute()
    return (dask_array_mean,)
...
def motable(
    ...
    rows = [
        ...
        benchmark("chunk-local (Charles Optimise - Mapreduce)", blockwise_mean, repeats.value),
        benchmark("inplace (Deepak Optimise )", inplace, repeats.value),
        # Same naive expression, but on dask-array: the optimiser fuses the elementwise
        # chain into the reduction, so we get the map-reduce win with no hand-built kernel.
        benchmark("algebraic (control)", algebraic, repeats.value),
        benchmark("dask-array (rewrite)", dask_array_mean, repeats.value),
    ]
...
def _(mo):
    ...
    We benchmark all five cases 10× per point and take the **median speedup vs naive**;
...
def _(
    ...
    # This case's inputs go through dask-array's chunk manager; register once up front.
    dexpr.xarray.register()
    ...
    def _cases(data):
        # Rebuild the five expressions for an arbitrary chunked DataArray.
        ...
        # dask-array: same naive expression, but on a dask-array-backed copy of the same
        # values. The optimiser fuses the elementwise chain into the reduction for us.
        _dexpr_x = xr.DataArray(
            dexpr.from_array(
                data.data.compute(), chunks=tuple(a[0] for a in data.data.chunks)
            ),
            dims=data.dims,
            name=data.name,
        ).persist()
        _dexpr_x.compute()
        _dask_array = ((_dexpr_x * 1.8) + 32).mean()

        _algebraic = data.mean() * 1.8 + 32
        return _naive, _chunk_local, _inplace, _dask_array, _algebraic
    ...
    sweep_dask_array = np.full_like(sweep_chunk_local, np.nan)
    ...
    for _i, _c in enumerate(sweep_chunk_edges):
        for _j, _k in enumerate(sweep_chunks):
            ...
            _x.compute()
            _naive_e, _chunk_e, _inplace_e, _dexpr_e, _alg_e = _cases(_x)
            ...
            sweep_dask_array[_i, _j] = _t_naive / _median_time(_dexpr_e, sweep_runs)
...
def benchmark_heatmap(
    ...
    _speedup_panels = [
        ...
        ("dask-array (rewrite)", sweep_dask_array),
        ("algebraic (control)", sweep_algebraic),
    ]
    # Shared colour scale across the speedup panels so they're directly comparable.
    _vmin = min(
        np.nanmin(sweep_chunk_local),
        np.nanmin(sweep_inplace),
        np.nanmin(sweep_dask_array),
        np.nanmin(sweep_algebraic),
    )
    _vmax = max(
        np.nanmax(sweep_chunk_local),
        np.nanmax(sweep_inplace),
        np.nanmax(sweep_dask_array),
    ...
    _fig, _axes = plt.subplots(1, 5, figsize=(26, 4.5), constrained_layout=True)
    ...
    _fig.colorbar(_im, ax=_axes[:4], label="speedup ×")
...
def _(mo):
    ...
    - **dask-array** -- the *same naive expression*, but its optimiser lowers the mean to
      a `PartialReduce` and fuses the elementwise chain into that reduction's per-block
      leaves. So the graph that runs is the chunk-local map-reduce — **read N, write ~0** —
      reached by rewriting rather than a hand-built kernel. It lands in the same
      memory class as chunk-local, which is the whole point: the win is structural, not
      something you have to hand-write per pattern.
