# W9 — Grouped and windowed contexts: the first sub-plan variant

> **SUPERSEDED (2026-07) by [`02-lowering.md`](./02-lowering.md).** The problem this memo
...
> recorder. Three specific reversals, argued in `02`:
>
> - **The `Contextual(inner: Op)` sub-plan shape is rejected** in favour of flat semantic
>   nodes (`02` §5.4). A context wrapping a call is a fact about how xarray *spells* a
...
>   start; only its pushdown *rule* is deferred, and for an unrelated reason (`02` §8.1).
> - **§2's scoping of `rolling`/`coarsen` as too hard is also wrong.** Rolling keeps its
>   dim at the same size, which makes its dim algebra *simpler* than groupby's (`02` §5.2).
>
> **§4 survives intact** and has moved to [`03-schema-sizes.md`](./03-schema-sizes.md) as
...
> [`03-schema-sizes.md`](./03-schema-sizes.md). Implement from there.*
...
- **`rolling`/`coarsen`/`weighted` are not covered** by this shape's dim algebra and
  ...
  > **Incorrect — see [`02-lowering.md`](./02-lowering.md) §5.2–§5.3.** "Weighted composes
