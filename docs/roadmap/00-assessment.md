...
| ~~9~~ | ~~**#121 — an emptying select crosses an auto-sizing rechunk**~~ — **done 2026-07-31.** `pushdown_selects_past_rechunks` refuses the hop when the rechunk is auto-sizing (`_auto_sizing`, over `ChunkSpec`) and the select cannot be vouched for as non-emptying (`_may_empty`, over `Indexer`) — two `assert_never` sites, and **no extent read at all**: emptiness is a fact about the indexer, not the schema, so `sizes` stays the field no rewrite reads. The generator exclusion is gone, which was the acceptance test. Repairing the hop instead — injecting `"auto"` for the emptied dim, as [xarray #11486](https://github.com/pydata/xarray/pull/11486) does — was measured and does not work: `auto_chunks` pins the empty dim and recurses, so a `300x0x300` dataset raises even with every dim `"auto"`. The same measurement found the uniform `chunk("auto")` half of the bug, which the first fix missed. Fixing it surfaced **#125** (see item 10). | issue #121 | Was the only case in the package where the rewrite was worse than eager rather than merely different (contrast `07-small-wins.md` §8). |
| ~~10~~ | ~~**#125 — `apply_schema` under-reports dims after a weighted reduce**~~ — **done 2026-07-31.** The `WeightedReduce` arm subtracted the dims *any* variable already carried before minting, so `weighted(w(time)).mean("lat")` left `elevation` 0-d on a dataset whose `temperature` still spanned `time`. Presence in some variable is not presence in each; `_minted` was already per-variable and idempotent, so the fix was to stop subtracting. Found by `test_tracked_schema_agrees_with_evaluation` while fixing #121, and predating it. | issue #125 | Under-reporting dims is the unsafe direction, though no rule acted on it: `pushdown_projections` reads `var_dims` only to *refuse*. |
...
and #125 later still — as items 9 and 10, since they belong to no family and the numbering
is a reading order rather than a priority. Anything
