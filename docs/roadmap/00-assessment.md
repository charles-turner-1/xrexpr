...
| ~~9~~ | ~~**#121 — an emptying select crosses an auto-sizing rechunk**~~ — **done 2026-07-31.** `pushdown_selects_past_rechunks` refuses the hop when the rechunk is auto-sizing (`_auto_sizing`, over `ChunkSpec`) and the select cannot be vouched for as non-emptying (`_may_empty`, over `Indexer`) — two `assert_never` sites, and **no extent read at all**: emptiness is a fact about the indexer, so `sizes` stays the field no rewrite reads. The generator exclusion is gone, which was the acceptance test. Refused rather than repaired because no chunk spec repairs it — naming the emptied dim `"auto"`, as [xarray #11486](https://github.com/pydata/xarray/pull/11486) does, still divides by zero once any dim exceeds `limit ** (1 / ndim)`. Covers the uniform `chunk("auto")` as well as the mapping form; both raise, and only the fixture size hid it. Fixing it surfaced **#125** (see item 10). | issue #121 | Was the only case in the package where the rewrite was worse than eager rather than merely different (contrast `07-small-wins.md` §8). |
