...
| ~~9~~ | ~~**#121 — an emptying select crosses an auto-sizing rechunk**~~ — **done 2026-07-31.** `pushdown_selects_past_rechunks` now sizes the select against the folded schema and refuses the hop when an extent would reach 0, but only for the `Auto`/`ByteSize` specs dask resolves by dividing by the array's own extent (`_auto_sizing`, the taxonomy's second `assert_never` site). The generator exclusion is gone, which was the acceptance test. It cost one invariant, restated in the four places that asserted it: **exactly one rule reads an extent, and only to withhold a rewrite** — never to license one, with "don't know" read as empty. Fixing it surfaced **#125** (see item 10). | issue #121 | Was the only case in the package where the rewrite was worse than eager rather than merely different (contrast `07-small-wins.md` §8). |
