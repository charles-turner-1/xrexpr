...
fold a schema forward as it records and resolve things eagerly — and it must not, because
...
`__getitem__` is *indexing* rather than projection — and which applies is read off the
