...
holds the **real dataset** (`self._base_ds`), each op is normalised into an `OpNode` *at
record time*, resolving dims/coords authoritatively against the live schema — something the
CST approach could never do. This is where the demo's `_optimize_ops` (which re-pokes raw
`args`/`kwargs` to guess dims) should move to:
...
B6.) Each `OpNode` carries `kind` (`reduce` | `scan` | `select` | `elementwise` | `opaque`),
`consumes` (dims removed), and `indexer` (for selects). Rules run to a **fixpoint**:
...
- **Optional cost model:** annotate each `OpNode` with an output-size estimate (dim sizes /
  chunk sizes) and push selections down greedily to minimise elements touched (eager) or
  task count/size (dask) — a principled objective instead of a single hard-coded swap.
...
- `ds.opt.…….explain()` → pretty-print the optimised op list (and the cost estimate), for
  debugging. Deterministic, unlike timing.
...
- `accessor.py` (from `add-accessor`) — the `@xr.register_dataset_accessor` proxy. Keep its
  ...
  **Change:** `_record` normalises each call into an `OpNode` (below) using the live
  `self._base_ds`, instead of storing raw `(name, args, kwargs)` tuples; `compute()` calls
  `optimize()` then replays. Add `explain()`.
- `ir.py` — a single flat record type (no tree):
  ...
  class OpNode:
      ...
      consumes: frozenset[str]     # dims removed — resolved vs the real dataset
      indexer: dict                # for selects: {dim: indexer}
  ```
  A helper `to_opnode(base_ds, name, args, kwargs) -> OpNode` does the record-time
  resolution (handles positional/keyword/tuple dims and no-dim `mean()` → all dims).
...
- **Property-based (Hypothesis) — the headline addition.** Generate random small datasets +
  random *legal* accessor chains; assert for all: (a) `ds.opt.<chain>.compute()` equals the
  eager `ds.<chain>` (correctness), and (b) the cost metric never increases (optimizer is
...
1. Merge/rebase `add-accessor`. Introduce `OpNode` + `to_opnode`; have `_record` build
   `OpNode`s and lift `_optimize_ops` verbatim into `optimize.py`. Behaviour unchanged; just
   restructured. Tests stay green.
...
- **Multi-input ops → the list becomes a tree.** Binary arithmetic (`ds.opt.mean("x") -
  ds.opt.mean("y")`), `concat`, and `where(expr)` have more than one input, so the flat
  `[OpNode]` list generalises to a DAG. Until modelled, treat these as `opaque` barriers
  (don't reorder across them); this is the one place the "tree" framing eventually returns.
- **Weighted reductions** (`ds.weighted(w).mean(...)`): pushing a slice must slice the
  weights too — model `weighted` as an `OpNode` so the rule knows. Until modelled, treat as
  a barrier.
- **Order-dependent ops** (`cumsum`, `cumprod`, `diff`, `.rolling`): `kind="scan"` — do
  **not** commute with a slice on their own dim; leave in place, never raise.
- **`sel` specifics**: label indexers, `slice` labels, `method="nearest"`, tolerance —
  ...
  (record-time resolution handles the coordinate lookup).
...
- **Value-dependent control flow**: forces materialisation — the recorded chain ends at the
  branch and a fresh one starts after it.
